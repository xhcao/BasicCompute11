...
HRESULT CreateTexture2D(_In_ ID3D11Device* pDevice, _In_ UINT xDimension, _In_ UINT yDimension, bool readonly, _In_ float* initialData, _Outptr_ ID3D11Texture2D** pTexture2D);
...
int __cdecl main()
{
	...
	matrixA = new float[NUM_ELEMENTS];
	matrixB = new float[NUM_ELEMENTS];
	matrixC = new float[NUM_ELEMENTS];
	matrixReference = new float[NUM_ELEMENTS];
	...
	UINT dispatchX = xDimension / tiledSize;
	UINT dispatchY = yDimension / tiledSize;
    RunComputeShader( g_pContext, g_pCS, 2, aRViews, nullptr, nullptr, 0, g_pMatrixCUAV, dispatchX, dispatchY, 1);
    printf( "done\n" );
...
HRESULT CreateTexture2D(_In_ ID3D11Device* pDevice, _In_ UINT xDimension, _In_ UINT yDimension, bool readonly, _In_ float* initialData, _Outptr_ ID3D11Texture2D** pTexture2D)
{
	...
	TexData.SysMemPitch = xDimension * sizeof(float);
